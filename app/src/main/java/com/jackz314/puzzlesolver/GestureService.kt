...
class GestureService : AccessibilityService() {
    ...
    private val handler = HandlerThread("GestureService").apply{start()}.run {HandlerCompat.createAsync(looper)}
    ...
    private fun clickGesture(x: Float, y: Float): GestureDescription {
        val clickPath = GestureDescription.StrokeDescription(Path().apply{moveTo(x,y)}, 0, 5)//1 ms is enough for click
...
//        Log.d(TAG, "swipe: $x, $y | $dx, $dy")
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        Log.d(TAG, "onAccessibilityEvent: GOT AccessibilityEvent: $event")
    }
